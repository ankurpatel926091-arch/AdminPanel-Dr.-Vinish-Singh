...
export const AuthProvider = ({ children }) => {
  const [user, setUser] = useState(() => {
    try {
      ...
      if (parsed) {
        parsed.role =
          parsed.role === "Administrator" || parsed.role === "admin"
            ? "admin"
            : null;
      }
      return parsed;
  ...
  const [token, setToken] = useState(
    () => localStorage.getItem("dr_vinish_admin_token") || null,
  );
  const [isAuthenticated, setIsAuthenticated] = useState(() =>
    Boolean(localStorage.getItem("dr_vinish_admin_token")),
  );
  ...
  const normalizeUserData = (rawUser) => {
    ...
    const normalizedRole = rawUser.role?.toLowerCase();
